...
主题设置中的“延迟显示地区”可以限定要显示的测试地区。选择 `广东` 后，仅汇总任务名称中包含“广东”的电信、联通、移动任务，卡片右上角也会显示“广东”；选择 `全部` 则汇总所有地区。选择 `自定义` 后，在下方填写城市或任务名中的区域标签，例如 `广州`、`杭州`。地区使用任务名称关键词匹配，因此不需要修改后端接口。

数据读取优先使用 Komari 的 Metric 接口，旧版实例会自动回退到 Ping 历史接口。点击延迟或丢包面板仍可打开节点的完整 Ping 监测详情。

---

## 🛰️ v3.3.18 NextTrace 与完整回程视图

- TZA Probe Agent 的回程探测优先使用固定版本 NextTrace，按每跳 ASN 区分 `CMIN2` 与 `CMI`；NextTrace 不可用时自动降级到系统 traceroute。
- 探测默认使用 30 秒总预算、每跳 2.5 秒和多次采样；跨境丢包或慢响应不会因为 8 秒短超时过早截断。未到达目标时也会保留已获得的最长路径并明确标记超时。
- 节点卡片的延迟、丢包、IPv4/IPv6 和回程线路标签整体放大，长任务名与组合线路不再省略。
- Trace 详情改为不透明的屏幕级大浮层，地址仍保持脱敏；全部跳点一次显示，窗口高度不足时仅在浮层内滚动，不分页，页面滚动不会拖动浮层位置。
- 节点卡片统一使用 comfortable 布局，移除容易造成挤压的多档尺寸切换。

---

## 🧩 v3.3.15 监控摘要与任务排序

- 节点卡片的三网摘要改为“运营商 / IPv4 / IPv6”稳定三列，增加可用高度并压缩重复空状态，窄卡片不再裁切协议族数据。
- 延迟与回程共用更紧凑的标题栏；回程只展示线路类型，例如 `CN2GIA`、`10099->4837`，延迟和丢包仍归延迟监控显示。
- TZA-Web 监控中心的延迟任务与回程任务均支持拖拽排序；延迟顺序写入 Ping 任务权重，回程顺序按任务配置数组持久化。
- 监控任务表格采用固定列宽和横向滚动，名称、服务器、目标及操作按钮不再互相挤压或重叠。
...
- 节点卡片把延迟、丢包和回程合并为固定高度的探测摘要；IPv4/IPv6 分栏、三网固定按电信/联通/移动排序，多地区回程在卡片内滚动，不再引起整页重排。
...
- 目标目录随 TZA Probe Core 本地内置，不依赖在线地址。该后台需要配套包含 `admin:setCarrierPingSelections` 的新版 TZA Probe Core。
...
检测服务应为每个节点维护定时任务，仅允许配置好的三网目标，单任务设置连接/路由超时并限制并发；TZA Probe Agent 使用本地运营商前缀规则生成线路标签，Core 持久化最近一次结构化结果。后端未实现此 RPC 时，主题保留空状态，不会把普通 Ping 结果当作回程线路。
...
- 节点卡片统一使用 comfortable 尺寸，保证三网摘要和资源指标始终有稳定空间
