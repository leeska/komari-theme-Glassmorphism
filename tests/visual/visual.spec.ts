...
test('node cards show the three China carrier ping panels', async ({ page }) => {
  ...
  for (const [index, carrier] of ['mobile', 'unicom', 'telecom'].entries())
...
test('node cards filter China carrier ping tasks by configured region', async ({ page }) => {
  ...
  await expect(latencyRows.nth(0)).toContainText('移动')
  await expect(latencyRows.nth(0)).toContainText('140 ms')
  await expect(latencyRows.nth(0)).toHaveAttribute('title', /广东移动/)
  ...
  await expect(latencyRows.nth(2)).toContainText('电信')
  await expect(latencyRows.nth(2)).toContainText('130 ms')
  await expect(latencyRows.nth(2)).toHaveAttribute('title', /广东电信/)
